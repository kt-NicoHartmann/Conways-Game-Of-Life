(function() {
    ...
    let COLS, ROWS, grid, nextGrid, gen = 0,
        running = false,
        rafId = null,
        lastTime = 0,
        fps = 10;
    ...
    function cellSize() {
        return Math.round(BASE_CELL * ZOOM_STEPS[zoomIdx]);
    }
    ...
    function random() {
        ...
        gen = 0;
        for (let r = 0; r < ROWS; r++)
            for (let c = 0; c < COLS; c++) grid[r][c] = Math.random() < 0.3 ? 1 : 0;
        draw();
    }

    function stamp(pattern, offR, offC) {
        pattern.forEach(([r, c]) => {
            const nr = (r + offR + ROWS) % ROWS,
                nc = (c + offC + COLS) % COLS;
            grid[nr][nc] = 1;
        });
    }
    ...
    wrap.addEventListener('wheel', e => {
        if (e.ctrlKey || e.metaKey) {
            e.preventDefault();
            if (e.deltaY < 0 && zoomIdx < ZOOM_STEPS.length - 1) {
                zoomIdx++;
                updateZoom();
